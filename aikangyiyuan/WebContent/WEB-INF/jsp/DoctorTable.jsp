...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/doctorAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("doctor/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="doctor/informationLoad" method="POST"
		onsubmit="return check();">
	<%-- 	<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>模糊搜索</label><input type="text" class="scinput1"
						name="searchName" value="${doctor['searchName'] }"></li>
		...
		<div class="rightinfo">
			...
			<table class="imgtable">
				...
				<tbody>
					...
					<c:forEach items="${doctorList.datas }" var="doctor" varStatus="vg">
						<tr>
							<td>
								<div id="images">
								<img src="upload/upload/${doctor['url'] }">
								</div>
							</td>
							<td><c:out value="${doctor['name'] }"></c:out></td>
							<td>
								<c:if test="${doctor['sex'] eq 1}">
									<c:out value="男" />
								</c:if>
								<c:if test="${doctor['sex'] eq 2}">
									<c:out value="女" />
								</c:if>
							</td>
							<td><c:out value="${doctor['age'] }"></c:out></td>
							<td><c:out value="${doctor['price'] }"></c:out></td>
							<td>
								<c:if test="${doctor['type'] eq 1}">
									<c:out value="医师" />
								</c:if>
								<c:if test="${doctor['type'] eq 2}">
									<c:out value="专家" />
								</c:if>
							</td>
							<td><c:out value="${doctor['part']['name'] }"></c:out></td>
							<td><c:out value="${doctor['createtime'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${doctor['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${doctor['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="doctor/informationLoad?pageNow=${pager.pageNow }&searchName=${doctor['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="doctor/informationLoad?pageNow=${pager.pageNow-1 }&searchName=${doctor['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="doctor/informationLoad?pageNow=${vs.count }&searchName=${doctor['searchName']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="doctor/informationLoad?pageNow=${count }&searchName=${doctor['searchName']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="doctor/informationLoad?pageNow=${count }&searchName=${doctor['searchName']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="doctor/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
