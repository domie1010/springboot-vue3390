...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/partAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("part/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="part/informationLoad" method="POST"
		onsubmit="return check();">
	<%-- 	<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>模糊搜索</label><input type="text" class="scinput1"
						name="searchName" value="${part['searchName'] }"></li>
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${partList.datas }" var="part" varStatus="vg">
						<tr>
							<td><c:out value="${part['name'] }"></c:out></td>
							<td><a href="javascript:;" onclick="update(${part['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${part['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="part/informationLoad?pageNow=${pager.pageNow }&searchName=${part['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="part/informationLoad?pageNow=${pager.pageNow-1 }&searchName=${part['searchName']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="part/informationLoad?pageNow=${vs.count }&searchName=${part['searchName']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="part/informationLoad?pageNow=${count }&searchName=${part['searchName']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="part/informationLoad?pageNow=${count }&searchName=${part['searchName']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="part/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
