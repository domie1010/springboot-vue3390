...
<script type=text/javascript src="js/js/foreground/doctort.js"></script>
...
<c:if test="${sessionScope['User'] ne null and sessionScope['User'] ne '' }">
	<a href="part/informationLoadA" target="_self" class="channelmenu_main" id="channelmenu_main_1" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','1')">医师预约</a>

	
	<a href="part/informationLoadB" target="_self" class="channelmenu_main" id="channelmenu_main_4" onMouseOver="showSubMenu('channelmenu_main','channelmenu_sub','4')">专家预约</a>
...
<c:forEach items="${pager.datas }" var="doctor" varStatus="vs">
<div class="shopquery">
		<div class="fang" style="width:120px;height:120px">
			<div class="picFit" style="width:120px;height:120px">
			<a href="doctor/informationLoadOneFore?id=${doctor['id'] }" target="_self" ><img src="upload/upload/${doctor['url'] }" style="width:120px;height:120px" border="0" /></a>
			</div>
		</div>
		<div class="prop">
		价格：<span class="hb">￥</span><span class="price">${doctor['price'] }</span><br />
		</div>
		<a href="doctor/informationLoadOneFore?id=${doctor['id'] }" target="_self" class="prodtitle"   >${doctor['name'] }</a>
		<div class="memo" >
		${doctor['part']['name'] }
		</div>
</div>
...
<div id="showpages" >
	  ...
	  <a href="doctor/informationLoadFore?pageNow=${pager.pageNow }">上一页</a>
	  </c:if>
	   <c:if test="${!pagele}">
	  <a href="doctor/informationLoadFore?pageNow=${pager.pageNow -1}">上一页</a>
	  </c:if>
	  </li>
	  <li class="pbutton">
	  <c:if test="${pager.pageNow ge pager.totalPages }" var="pagege">
	  <a href="doctor/informationLoadFore?pageNow=${pager.pageNow }">
	  下一页
	  </a>
	  </c:if>
	  <c:if test="${!pagege }">
	  <a href="doctor/informationLoadFore?pageNow=${pager.pageNow+1 }">
	  下一页
	  </a>
	  </c:if>
	  </li></ul></div>
